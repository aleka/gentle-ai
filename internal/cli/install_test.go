...
func TestNormalizeInstallFlagsDefaults(t *testing.T) {
	...
	// When no --agents flag is given, agents are resolved from state.json
	// (Priority 1) or filesystem detection (Priority 2). The exact set
	// depends on the caller's environment, so we only assert the defaults
	// that are always constant regardless of agent selection.
	if input.Selection.Persona != model.PersonaGentleman {
		t.Fatalf("Persona = %q, want %q", input.Selection.Persona, model.PersonaGentleman)
	}
	if input.Selection.Preset != model.PresetFullGentleman {
		t.Fatalf("Preset = %q, want %q", input.Selection.Preset, model.PresetFullGentleman)
	}
